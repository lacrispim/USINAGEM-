...
export default function ProgrammingPage() {
  ...
  useEffect(() => {
    // Só carrega do banco se não houver uma atualização local pendente
    const now = Date.now();
  ...
  const toggleConcluded = useCallback(async (itemId: string) => {
    ...
    try { 
      await setDoc(doc(firestore, 'programacaoState', 'plano'), { data: updatedPlano, updatedAt: serverTimestamp() }); 
    } catch (e) { toast({ title: "Erro", description: "Falha ao salvar status.", variant: "destructive" }); }
  ...
  const recalculatePlan = async (novaFila: JobBase[], currentDisabled = disabledShifts, currentOverrides = techOverrides, anchor?: Date) => {
    ...
    (lossRecords || []).forEach(loss => {
        ...
        if (!shiftId) shiftId = getShiftFromDate(loss.createdAt?.toDate ? loss.createdAt.toDate() : d);
    ...
    for (let dIdx = 0; dIdx < 365; dIdx++) {
        ...
        ['1', '2', '3'].forEach((sId, sIdx) => {
            ...
            ['TORNO', 'CENTRO', 'ADM'].forEach(tk => {
                ...
                if (lossData && lossData.total > 0) {
                    occupy(laneId, shiftAbs, shiftAbs + lossData.total);
                    novosPlanItems.push({
                        id: `loss-${techName}-${dStr}-${sId}`, dataExecucao: format(d, 'dd/MM/yyyy'), tecnico: techName, equipamento: 'PERDA', requisicao: 'PERDA',
                        nomeDaPeca: lossData.descriptions, quantidadeTotal: 0, quantidadeNoBloco: 0, tempoMinutos: lossData.total, setupMinutos: 0, turno: sId,
                        startOffsetMin: 0, tipoAtividade: 'PERDA', techKey: tk as any, jobId: 'loss', laneIndex: 0, isConcluded: true, site: 'SISTEMA'
                    });
                }
            });
        });
    ...
    const allocateTask = (job: JobBase, techKey: 'TORNO' | 'CENTRO' | 'ADM', minStartTime: number, type: 'torno' | 'centro' | 'prog') => {
        ...
        while ((pendingSetup > 0.01 || pendingProd > 0.01) && iter < 3000) {
            ...
            if (sInShift > 0 || pInShift > 0) {
                ...
                novosPlanItems.push({ 
                    id: `pl-${job.id}-${techKey}-${dateStr}-${shiftId}-${Math.round(winStart)}`, 
                    dataExecucao: displayDateStr, tecnico: techName, equipamento: type.toUpperCase(), 
                    ...
                    jobId: job.id, laneIndex: 0, isConcluded: concluidos.has(`${job.id}|${techKey}|${displayDateStr}|${shiftId}`), site: normalizeSiteName(job.site)
                });
                cursor = abs + duration;
    ...
    [...novaFila].filter(j => j.etapa1 === 'TORNO' || j.etapa2 === 'TORNO')
      .sort((a, b) => (a.ordemTorno || 999) - (b.ordemTorno || 999))
      .forEach(j => finishTimes[j.id] = allocateTask(j, 'TORNO', j.etapa1 === 'TORNO' ? 0 : (finishTimes[j.id] || 0), 'torno'));

    [...novaFila].filter(j => j.etapa1 === 'CENTRO' || j.etapa2 === 'CENTRO')
      .sort((a, b) => (a.ordemCentro || 999) - (b.ordemCentro || 999))
      .forEach(j => finishTimes[j.id] = allocateTask(j, 'CENTRO', j.etapa1 === 'CENTRO' ? 0 : (finishTimes[j.id] || 0), 'centro'));
    ...
    } catch (e) { toast({ title: "Erro", description: "Falha ao gravar cronograma.", variant: "destructive" }); }
  ...
  const handleSetAnchorDate = async (date: Date | undefined) => {
    ...
    try {
      await setDoc(doc(firestore, 'programacaoState', 'config'), { planStartDate: format(selectedDate, 'yyyy-MM-dd') }, { merge: true });
      await recalculatePlan(fila, disabledShifts, techOverrides, selectedDate);
    } catch (e) { toast({ title: "Erro", description: "Falha ao salvar âncora.", variant: "destructive" }); }
  };
  ...
  const moveJob = async (currentIdx: number, newPos: number, type: 'GERAL' | 'TORNO' | 'CENTRO') => {
    const newFila = [...fila];
    const item = newFila[currentIdx];
    const target = Math.max(0, Math.min(newFila.length - 1, newPos - 1));

    if (type === 'GERAL') {
      const [removed] = newFila.splice(currentIdx, 1);
      newFila.splice(target, 0, removed);
    } else {
      ...
      const filteredIdx = list.findIndex(j => j.id === item.id);
      const [removed] = list.splice(filteredIdx, 1);
  ...
  const handleAddManual = async () => {
    ...
    const job: JobBase = {
      id: `job-${Date.now()}`, requisicao: newItem.requisicao!, nomeDaPeca: newItem.nomeDaPeca!.toUpperCase(),
      quantidade: Number(newItem.quantidade) || 1, setup: Number(newItem.setup) || 20, torno: Number(newItem.torno) || 0,
      centro: Number(newItem.centro) || 0, prog: Number(newItem.prog) || 0, site: newItem.site || 'VALINHOS',
      etapa1: newItem.etapa1 || 'TORNO', etapa2: newItem.etapa2 || '', turnoDesejado: '',
      ordemTorno: fila.length + 1, ordemCentro: fila.length + 1
    };
    ...
    reader.onload = async (event) => {
      try {
        ...
        const findVal = (row: any, keys: string[]) => {
          for (const k of keys) { const rk = Object.keys(row).find(x => x.toLowerCase().trim() === k.toLowerCase().trim()); if (rk) return row[rk]; }
          return undefined;
        };
        const novaFila: JobBase[] = json.map((row, i) => ({
          id: `job-imp-${i}-${Date.now()}`, requisicao: String(findVal(row, ['requisição', 'requisicao', 'req', 'forms']) || 'S/N'),
          nomeDaPeca: String(findVal(row, ['peça', 'peca', 'nome']) || 'SEM NOME').toUpperCase(), quantidade: Number(findVal(row, ['qtd', 'quantidade']) || 1),
          setup: Number(findVal(row, ['setup']) || 20), torno: Number(findVal(row, ['torno']) || 0), centro: Number(findVal(row, ['centro']) || 0),
          prog: Number(findVal(row, ['prog', 'programação']) || 0), site: normalizeSiteName(String(findVal(row, ['site', 'fabrica']) || 'VALINHOS')),
          etapa1: String(findVal(row, ['etapa 1', 'etapa1']) || 'TORNO'), etapa2: String(findVal(row, ['etapa 2', 'etapa2']) || ''),
  ...
  const renderFilaTable = (jobs: JobBase[], type: 'GERAL' | 'TORNO' | 'CENTRO') => (
    <Table>
      ...
      <TableBody>
        {jobs.length === 0 ? (
          <TableRow><TableCell colSpan={11} className="text-center py-10 opacity-30 italic">Nenhuma requisição encontrada</TableCell></TableRow>
        ) : jobs.map((job, idx) => {
          const stats = jobCompletionStats.get(job.id);
          const isDone = stats && stats.total > 0 && stats.total === stats.concluded;
          ...
          return (
            <TableRow key={job.id} className={cn(isDone && "bg-green-500/5 opacity-60")}>
              <TableCell className="text-center">
                <Input type="number" defaultValue={pos} className="h-8 w-14 text-center font-black" onBlur={(e) => moveJob(idx, Number(e.target.value), type)}/>
              </TableCell>
              <TableCell>
                <div className="flex flex-col gap-1">
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveJob(idx, pos - 1, type)} disabled={idx === 0}><ArrowUp className="h-3" /></Button>
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => moveJob(idx, pos + 1, type)} disabled={idx === jobs.length - 1}><ArrowDown className="h-3" /></Button>
                </div>
              </TableCell>
              <TableCell>
                {isDone ? <div className="text-green-500 font-black text-[10px]">CONCLUÍDO</div> : <div className="text-muted-foreground/40 font-black text-[10px]">PENDENTE</div>}
              </TableCell>
              ...
              <TableCell>
                <Select value={job.turnoDesejado || "AUTO"} onValueChange={(v) => updateJobField(job.id, 'turnoDesejado', v === "AUTO" ? "" : v)}>
                  <SelectTrigger className="h-8 text-[11px] font-black"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="AUTO">AUTO</SelectItem><SelectItem value="1">1T</SelectItem><SelectItem value="2">2T</SelectItem><SelectItem value="3">3T</SelectItem></SelectContent>
                </Select>
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-1">
                  <Button variant={job.etapa1 === 'TORNO' || job.etapa2 === 'TORNO' ? "default" : "outline"} size="sm" className="h-7 w-8 font-black text-[10px]" onClick={() => updateJobField(job.id, 'etapa1', job.etapa1 === 'TORNO' ? '' : 'TORNO')}>T</Button>
                  <Button variant={job.etapa1 === 'CENTRO' || job.etapa2 === 'CENTRO' ? "default" : "outline"} size="sm" className="h-7 w-8 font-black text-[10px]" onClick={() => updateJobField(job.id, 'etapa1', job.etapa1 === 'CENTRO' ? '' : 'CENTRO')}>C</Button>
                </div>
              </TableCell>
              ...
              <TableCell className="text-right text-[10px] font-bold">
                {type === 'TORNO' ? `${job.torno}m` : (type === 'CENTRO' ? `${job.centro}m` : `${job.torno + job.centro}m`)}
              </TableCell>
              <TableCell><Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => { setFila(fila.filter(f => f.id !== job.id)); recalculatePlan(fila.filter(f => f.id !== job.id)); }}><Trash2 className="h-4" /></Button></TableCell>
            </TableRow>
          );
        })}
      </TableBody>
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 border-b pb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Popover open={isAnchorPopoverOpen} onOpenChange={setIsAnchorPopoverOpen}>
            <PopoverTrigger asChild><Button variant="outline" className="h-10 font-black uppercase text-[11px]"><Anchor className="h-4 w-4 mr-2" /> {planStartDate ? `Início: ${format(planStartDate, 'dd/MM')}` : "Definir Âncora"}</Button></PopoverTrigger>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 bg-card/50 border rounded-lg p-1.5 w-full md:w-auto">
          ...
          <Select value={selectedSiteFilter} onValueChange={setSelectedSiteFilter}>
            <SelectTrigger className="h-9 w-[180px] text-[11px] font-black border-0 bg-transparent shadow-none focus:ring-0"><SelectValue placeholder="Fábrica" /></SelectTrigger>
            <SelectContent><SelectItem value="all">TODAS FÁBRICAS</SelectItem>{FACTORIES.map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent>
          </Select>
          ...
          <Select value={selectedEquipmentFilter} onValueChange={setSelectedEquipmentFilter}>
            <SelectTrigger className="h-9 w-[150px] text-[11px] font-black border-0 bg-transparent shadow-none focus:ring-0"><SelectValue placeholder="Equipamento" /></SelectTrigger>
            <SelectContent><SelectItem value="all">TODOS</SelectItem><SelectItem value="TORNO">TORNO</SelectItem><SelectItem value="CENTRO">CENTRO</SelectItem></SelectContent>
          </Select>
        </div>
        <div className="flex items-center bg-card/50 border rounded-lg p-1.5 w-full md:w-auto justify-center">
          <Button variant="ghost" size="icon" onClick={() => planStartDate && setCurrentDate(planStartDate)}><ChevronsLeft className="h-5 w-5" /></Button>
          <Button variant="ghost" size="icon" onClick={() => setCurrentDate(prev => addDays(prev, -1))}><ChevronLeft className="h-5 w-5" /></Button>
          <Popover>
            <PopoverTrigger asChild><Button variant="ghost" className="font-black px-4 text-[12px] min-w-[140px] text-primary"><CalendarDays className="h-4 w-4 mr-2" />{format(currentDate, 'dd/MM/yyyy')}</Button></PopoverTrigger>
            <PopoverContent className="w-auto p-0"><Calendar mode="single" locale={ptBR} selected={currentDate} onSelect={d => d && setCurrentDate(startOfDay(d))} disabled={isDomingo} initialFocus/></PopoverContent>
          </Popover>
      ...
      <div className="space-y-8">
        {[currentDate].map(day => {
          ...
          const dDisplay = format(day, 'dd/MM/yyyy');
          return (
            <div key={dStr} className="bg-card border rounded-lg overflow-hidden shadow-sm">
              <div className="bg-muted/10 p-4 border-b flex items-center justify-between">
                <div className="flex items-center gap-4"><span className="text-2xl font-bold">{format(day, 'dd · MM/yy')}</span><span className="text-[10px] font-black uppercase opacity-40">{format(day, 'EEEE', { locale: ptBR })}</span></div>
              </div>
              {TURNOS.map(t => {
                const shiftKey = `${dStr}_${t.id}`;
                ...
                return (
                  <div key={t.id} className={cn("grid grid-cols-[100px_1fr] border-b last:border-0", isDisabled && "bg-stripes")}>
                    <div className="p-4 flex flex-col items-center justify-center border-r bg-muted/5">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 mt-2" onClick={() => { const nd = { ...disabledShifts, [shiftKey]: !isDisabled }; setDisabledShifts(nd); setDoc(doc(firestore!, 'programacaoState', 'config'), { disabledShifts: nd }, { merge: true }); recalculatePlan(fila, nd); }}>{isDisabled ? <PowerOff className="h-4 text-destructive" /> : <Power className="h-4 text-green-500" />}</Button>
                    </div>
